...
scanner = market_scan.MarketScanner(lambda t: None, exchanges=["MEXC SPOT"])
...
def mk(symbol, rng, vol, accel, exchange="MEXC SPOT"):
...
res = market_scan.MarketScanner.early_candidates([warming, other] + pumps, exchange="MEXC SPOT")
...
strict = {c["symbol"] for c in market_scan.MarketScanner.early_candidates(
    batch, exchange="MEXC SPOT", exclude_majors=True)}
...
loose = {c["symbol"] for c in market_scan.MarketScanner.early_candidates(
    batch, exchange="MEXC SPOT", exclude_majors=False)}
...
res = market_scan.MarketScanner.early_candidates(batch, exchange="MEXC SPOT", exclude_majors=True)
...
ranked = market_scan.MarketScanner.early_candidates(
    [spot, oi_flat, oi_rising], exchange="MEXC SPOT", exclude_majors=False)
...
scanner2 = market_scan.MarketScanner(lambda t: None, exchanges=["MEXC SPOT"])
...
print("\nживой запрос к MEXC...")
rows = market_scan._fetch_mexc_spot()
